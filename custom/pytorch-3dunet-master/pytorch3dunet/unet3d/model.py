...
class MultiTaskResidualUNetSE3D(ResidualUNetSE3D):
    def __init__(self, in_channels,
        ...
        for task_name, out_channels in out_channels_dict.items():
            ...
            # Create final 1x1 convolution
            self.task_final_convs[task_name] = nn.Conv3d(self.f_maps[0], out_channels, 1)

            # Set appropriate activation
            if task_name == 'sheet':
                self.task_activations[task_name] = nn.Sigmoid() if final_sigmoid else nn.Softmax(dim=1)
            elif task_name == 'affinities':
                self.task_activations[task_name] = nn.Sigmoid()  # Affinities typically use sigmoid
            else:  # normals
                self.task_activations[task_name] = None  # No activation for normal prediction
    ...
    def forward(self, x):
        ...
        results = {}
        for task_name in self.task_decoders.keys():
            task_x = x

            for decoder, encoder_features in zip(self.task_decoders[task_name], encoders_features):
                task_x = decoder(encoder_features, task_x)

            task_x = self.task_final_convs[task_name](task_x)

            if not self.training and self.task_activations[task_name] is not None:
