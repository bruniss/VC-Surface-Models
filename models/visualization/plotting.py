...
def log_predictions_as_video(self, writer, inputs, targets_dict, outputs, epoch):
    ...
    """
    # We'll just log the first item in the batch for simplicity
    batch_idx = 0

    # Log the input as video if you like (assuming shape [B, C, D, H, W])
    inp_3d = inputs[batch_idx: batch_idx + 1]  # shape => [1, C, D, H, W]
    # Sometimes you want to normalize/clamp to [0..1] for nicer visualization
    # e.g. if it's CT range
    ...
    for t_name in self.tasks:
        ...
        # For "normals", you might want to shift from [-1..1] to [0..1], etc.
        # We'll do a basic clamp for demonstration
        t_pred_clamped = torch.clamp((t_pred + 1.0) / 2.0, 0, 1) if "normal" in t_name.lower() else torch.sigmoid(
            t_pred)

        writer.add_video(f"val_pred_{t_name}", t_pred_clamped, epoch, fps=4)

        # Label
        if t_name in targets_dict:
            t_lbl = targets_dict[t_name][batch_idx: batch_idx + 1].clone()
            if "normal" in t_name.lower():
                # shift [-1..1] => [0..1]
                t_lbl = torch.clamp((t_lbl + 1.0) / 2.0, 0, 1)
            else:
                t_lbl = torch.clamp(t_lbl, 0, 1)
            writer.add_video(f"val_label_{t_name}", t_lbl, epoch, fps=4)
